...
void Fly()
{
    ...
    {
        if (Gracz.Direction == Gracz.DirectionChoose::right)
            Gracz.m_Sprite.setPosition(Gracz.Baza - Parabola.nextPoint()->toV2f_xrev());
        else
            Gracz.m_Sprite.setPosition(Gracz.Baza - Parabola.nextPoint()->toV2f_x());
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            {
                dX = 6;
                Gracz.ChangeDirection(Gracz.DirectionChoose::right);
            };
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left))
            {
                dX = -6;
                Gracz.ChangeDirection(Gracz.DirectionChoose::left);
            };
            ...
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space) && (!Gracz.isJumping) && (Gracz.CanJump))
            {
                JumpStart();
            };
        }
        ...
        {
            if (Gracz.testCollisionDown(Platform))
            {
                onGround = true;
                Gracz.isJumping = false;
            };
            if (Gracz.testCollisionLeft(Platform))
            {
                leftWall = true;
                Gracz.isJumping = false;
            };
            if (Gracz.testCollisionRight(Platform))
            {
                rightWall = true;
                Gracz.isJumping = false;
            };
        ...
        if ((Gracz.isJumping) && (!onGround))
        {
            Fly();
        }; // Parabolic jump step

        if ((rightWall) && (!onGround))
        {
            Gracz.MoveRight(-6);
            Gracz.CanJump = false;
        }; // Slide from Edge
        if ((leftWall) && (!onGround))
        {
            Gracz.MoveRight(6);
            Gracz.CanJump = false;
        }; // Slide from Edge
        ...
        if ((dX > 0) && (!rightWall) && (onGround))
            Gracz.MoveRight(dX);
        if ((dX < 0) && (!leftWall) && (onGround))
            Gracz.MoveLeft(dX);

        if ((!onGround) && (!Gracz.isJumping))
        {
            Gravity();
            Gracz.CanJump = false;
        }
